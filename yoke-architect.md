...
1. **Blank** — an empty Task board to grow from. *Available now.*
2. **From a vertical** — a curated recipe from our library: independent auto shop, hospital patient throughput, field service, sales pipeline, compliance program, inventory operations, general ledger. Each one is a real Yoke App, not a wireframe. *Rolling out next.*
3. **Clone an installed Yoke** — start from one you already use. *Rolling out next.*
4. **Promote an app** — take an existing single-board app you've built and reshape it into a Yoke; we wrap your boards and pages into the draft for you. *Rolling out next.*
5. **From a brief** — type one sentence describing what you want; an AI drafts the full Yoke for you to edit. *Rolling out next, Pro+ only.*
...
For the FastYoke engineering team, the same wizard is engineered to also produce a **paste-able patch**: the same YokeDoc compiles to Rust constants, installer and uninstaller functions, JSON assets, integration tests, and a customer doc page — everything needed to ship the app as a public marketplace listing. The wizard's two outputs (your tenant's database, or a unified diff against our codebase) come from the same description through two different interpreters.

Stay with that sentence. It is the entire thesis.
...
None of this requires a rewrite. None of it requires a "Yoke 2.0." It is what falls out of treating the app as a value in the first place.
...
We did not call any of this "functional programming" during the design. We called it "the simplest thing that could possibly work." That is usually a tell.
