...
::feature-grid{columns="3"}
  ::feature-card{title="Drag-and-drop builder" summary="Build forms visually — text, email, number, date, file upload, signature, multi-select, and more. Reorder fields with a click; no code required."}
  ::
  ::feature-card{title="Multi-page flows" summary="Break long forms into pages with a built-in stepper, per-page progress, and back/next navigation. Higher completion rates than a wall of fields."}
  ::
  ::feature-card{title="Conditional routing" summary="Skip irrelevant pages based on earlier answers. Define rules visually — if Country is US, skip the VAT page — without writing JavaScript."}
  ::
  ::feature-card{title="Themes you control" summary="Match your brand with the theme picker, or fork a starter theme and tweak the tokens. No FastYoke watermark on your forms."}
  ::
  ::feature-card{title="PDF on submit" summary="Generate a styled PDF artifact for every submission — useful for legal-record flows like waivers, releases, and intake forms. Submitter gets a download link."}
  ::
  ::feature-card{title="File uploads + scanning" summary="Accept attachments with size + MIME-type validation. Every uploaded file is virus-scanned before submission lands. No extra setup."}
  ::
  ::feature-card{title="Submission triage" summary="Cross-form submissions index with search, project scope, and per-row actions for view, download, and workflow advance. Triage your inbox in one place."}
  ::
  ::feature-card{title="Public links + invites" summary="Generate a shareable public URL for any form, or issue limited-use invite tokens with expiry dates. Revoke access at any time."}
  ::
  ::feature-card{title="Your data, exportable" summary="Submissions land in a SQLite database you can export with one command. No vendor lock-in — your data is yours, in a portable format."}
  ::
::
...
::cta-button{to="/pricing" variant="secondary" size="lg"}
Compare plans
::
